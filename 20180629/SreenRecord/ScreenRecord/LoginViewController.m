...
- (void)viewDidLoad {
    ...
    if (![app isSupportScreenRecord]) {
        self.tipinfo.text = @"屏幕录制功能只支持iOS9，iOS10系统。iPhone5s以下机型性能较差";
    }
    
}
- (IBAction)onEditIdentifierEnd:(UITextField *)sender {
    if ([sender.text isEqualToString:@"adev0"])
    {
        self.usersig.text = kAdev0Sig;
    }
    else if ([sender.text isEqualToString:@"adev1"])
    {
        self.usersig.text = kAdev1Sig;
    }
}
