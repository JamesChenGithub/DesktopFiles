...
TLocalIPStack __local_ipstack_detect() {
...
TLocalIPStack local_ipstack_detect() {
    std::string log;
    return __local_ipstack_detect();
}

static void __local_info();

TLocalIPStack local_ipstack_detect_log() {
    __local_info();
    return __local_ipstack_detect();
}
