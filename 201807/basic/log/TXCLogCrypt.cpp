...
uint32_t TXCLogCrypt::GetHeaderLen() {
    return sizeof(char) * 3 + sizeof(uint16_t) + sizeof(uint32_t) * 2;
}

uint32_t TXCLogCrypt::GetTailerLen() {
    return sizeof(kMagicEnd);
}

void TXCLogCrypt::SetHeaderInfo(char* _data, bool _is_async) {
...
void TXCLogCrypt::SetTailerInfo(char* _data) {
    memcpy(_data, &kMagicEnd, sizeof(kMagicEnd));
}

uint32_t TXCLogCrypt::GetLogLen(const char*  const _data, size_t _len) {
...
void TXCLogCrypt::UpdateLogLen(char* _data, uint32_t _add_len) {
...
bool TXCLogCrypt::GetLogHour(const char* const _data, size_t _len, int& _begin_hour, int& _end_hour) {
...
void TXCLogCrypt::UpdateLogHour(char* _data) {
...
bool TXCLogCrypt::GetPeriodLogs(const char* const _log_path, int _begin_hour, int _end_hour, unsigned long& _begin_pos, unsigned long& _end_pos, std::string& _err_msg) {
...
void TXCLogCrypt::CryptSyncLog(const char* const _log_data, size_t _input_len, char* _output, size_t& _output_len) {
...
void TXCLogCrypt::CryptAsyncLog(const char* const _log_data, size_t _input_len, char* _output, size_t& _output_len) {
    _output_len = std::min(_input_len, _output_len);
    memcpy(_output, _log_data, _output_len);
}

bool TXCLogCrypt::Fix(char* _data, size_t _data_len, bool& _is_async, uint32_t& _raw_log_len) {
