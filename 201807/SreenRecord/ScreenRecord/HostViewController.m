...
- (void)onExitRoom
{
    ...
    if (app.isScreenRecording)
    {
        [[[ILiveSDK getInstance] getAVContext].videoCtrl enableScreenRecord:NO withMode:QAVRecordPreset864x480 complete:^(int result) {
...
- (void)onStartRecord
{
    QAVRecordPreset rec = QAVRecordPreset864x480;
    AppDelegate *app = (AppDelegate *)[UIApplication sharedApplication].delegate;
    if (![app isScreenRecording])
    {
        __weak typeof(self) ws = self;
