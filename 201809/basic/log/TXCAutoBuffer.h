...
#ifndef TXCAUTOBUFFER_H
#define TXCAUTOBUFFER_H
...
class TXCAutoBuffer {
  ...
  public:
    explicit TXCAutoBuffer(size_t _size = 128);
    explicit TXCAutoBuffer(void* _pbuffer, size_t _len, size_t _size = 128);
    explicit TXCAutoBuffer(const void* _pbuffer, size_t _len, size_t _size = 128);
    ~TXCAutoBuffer();
    ...
    size_t Read(TXCAutoBuffer& _rhs , size_t _len);

    size_t Read(off_t& _pos, void* _pbuffer, size_t _len) const;
    size_t Read(off_t& _pos, TXCAutoBuffer& _rhs, size_t _len) const;

    size_t Read(const off_t& _pos, void* _pbuffer, size_t _len) const;
    size_t Read(const off_t& _pos, TXCAutoBuffer& _rhs, size_t _len) const;
    ...
    void Attach(TXCAutoBuffer& _rhs);
  ...
  private:
    TXCAutoBuffer(const TXCAutoBuffer& _rhs);
    TXCAutoBuffer& operator = (const TXCAutoBuffer& _rhs);
...
//extern const TXCAutoBuffer KNullAtuoBuffer;

template <class S> class copy_wrapper_helper;

template <>
class copy_wrapper_helper<TXCAutoBuffer> {
  public:
    static void copy_constructor(TXCAutoBuffer& _lhs, TXCAutoBuffer& _rhs)
    { _lhs.Attach(_rhs); }

    static void copy_constructor(TXCAutoBuffer& _lhs, const TXCAutoBuffer& _rhs)
    { _lhs.Attach(const_cast<TXCAutoBuffer&>(_rhs)); }

    static void destructor(TXCAutoBuffer& _delobj) {}
};

#endif	//TXCAUTOBUFFER_H
