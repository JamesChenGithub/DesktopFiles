...
static void __ComLogV(int level, const char *tag, const char* file, const char* func, int line, const char* fmt, va_list args) {
	...
	txf_logger_vprint(&info, fmt, args);

	//important: do not use like  this:txclogger2((TLogLevel)level, tag, file, func, line, fmt, args);
	//txclogger2((TLogLevel)level, tag, file, func, line).VPrintf(fmt, args);
}
...
#define __LOG__(LEVEL, LOG_TAG, FMT, ...)  if ((!txclogger_IsEnabledFor(LEVEL))); else __ComLog(LEVEL, LOG_TAG , __FILE__, __FUNCTION__, __LINE__, FMT,## __VA_ARGS__)
#define __LOGV__(LEVEL, LOG_TAG, FMT, VA_LIST)  if ((!txclogger_IsEnabledFor(LEVEL))); else __ComLogV(LEVEL, LOG_TAG , __FILE__, __FUNCTION__, __LINE__, FMT, VA_LIST)
...
#define __android_log_assert(COND, TAG, FMT, ...)	if (((COND) || !txclogger_IsEnabledFor(kLevelFatal)));else {\
                                   					TXSLogInfo info= {kLevelFatal, TAG, __FILE__, __FUNCTION__, __LINE__,\
                                    				{0, 0}, -1, -1, -1};\
                                    				gettimeofday(&info.timeval, NULL);\
                                    				txf_logger_assert_p(&info, #COND, FMT, ##__VA_ARGS__);}
