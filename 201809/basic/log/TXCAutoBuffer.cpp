...
//const TXCAutoBuffer KNullAtuoBuffer;
...
TXCAutoBuffer::TXCAutoBuffer(size_t _nSize)
    : parray_(NULL)
    , pos_(0)
    , length_(0)
    , capacity_(0)
...
TXCAutoBuffer::TXCAutoBuffer(void* _pbuffer, size_t _len, size_t _nSize)
    : parray_(NULL)
    , pos_(0)
    , length_(0)
    , capacity_(0)
...
TXCAutoBuffer::TXCAutoBuffer(const void* _pbuffer, size_t _len, size_t _nSize)
    : parray_(NULL)
    , pos_(0)
    , length_(0)
    , capacity_(0)
...
TXCAutoBuffer::~TXCAutoBuffer() {
    Reset();
}

void TXCAutoBuffer::AllocWrite(size_t _readytowrite, bool _changelength) {
...
void TXCAutoBuffer::AddCapacity(size_t _len) {
    __FitSize(Capacity() + _len);
}

void TXCAutoBuffer::Write(const void* _pbuffer, size_t _len) {
    Write(Pos(), _pbuffer, _len);
    Seek(_len, ESeekCur);
}

void TXCAutoBuffer::Write(off_t& _pos, const void* _pbuffer, size_t _len) {
    Write((const off_t&) _pos,  _pbuffer,  _len);
    _pos += _len;
}

void TXCAutoBuffer::Write(const off_t& _pos, const void* _pbuffer, size_t _len) {
...
void TXCAutoBuffer::Write(TSeek _seek, const void* _pbuffer, size_t _len){
    off_t pos = 0;
    switch (_seek) {
        case ESeekStart:
            pos = 0;
...
size_t TXCAutoBuffer::Read(void* _pbuffer, size_t _len) {
    size_t readlen = Read(Pos(), _pbuffer, _len);
    Seek(readlen, ESeekCur);
    return readlen;
}

size_t TXCAutoBuffer::Read(TXCAutoBuffer& _rhs , size_t _len) {
    size_t readlen = Read(Pos(), _rhs, _len);
    Seek(readlen, ESeekCur);
    return readlen;
}

size_t TXCAutoBuffer::Read(off_t& _pos, void* _pbuffer, size_t _len) const {
    size_t readlen = Read((const off_t&) _pos, _pbuffer,  _len);
    _pos += readlen;
    return readlen;
}

size_t TXCAutoBuffer::Read(off_t& _pos, TXCAutoBuffer& _rhs, size_t _len) const {
    size_t readlen = Read((const off_t&) _pos, _rhs,  _len);
    _pos += readlen;
    return readlen;
}

size_t TXCAutoBuffer::Read(const off_t& _pos, void* _pbuffer, size_t _len) const {
...
size_t TXCAutoBuffer::Read(const off_t& _pos, TXCAutoBuffer& _rhs, size_t _len) const {
    size_t readlen = Length() - _pos;
    readlen = min(readlen, _len);
...
off_t TXCAutoBuffer::Move(off_t _move_len) {
    if (0 < _move_len) {
...
void  TXCAutoBuffer::Seek(off_t _offset,  TSeek _eorigin) {
    switch (_eorigin) {
    case ESeekStart:
        pos_ = _offset;
        break;
...
void TXCAutoBuffer::Length(off_t _pos, size_t _lenght) {
    ASSERT(0 <= _pos);
...
void*  TXCAutoBuffer::Ptr(off_t _offset) {
    return (char*)parray_ + _offset;
}

const void*  TXCAutoBuffer::Ptr(off_t _offset) const {
    return (const char*)parray_ + _offset;
}

void* TXCAutoBuffer::PosPtr() {
    return ((unsigned char*)Ptr()) + Pos();
}

const void* TXCAutoBuffer::PosPtr() const {
    return ((unsigned char*)Ptr()) + Pos();
}

off_t TXCAutoBuffer::Pos() const {
    return pos_;
}

size_t TXCAutoBuffer::PosLength() const {
    return length_ - pos_;
}

size_t TXCAutoBuffer::Length() const {
    return length_;
}

size_t TXCAutoBuffer::Capacity() const {
    return capacity_;
}

void TXCAutoBuffer::Attach(void* _pbuffer, size_t _len) {
    Reset();
...
void TXCAutoBuffer::Attach(TXCAutoBuffer& _rhs) {
    Reset();
...
void* TXCAutoBuffer::Detach(size_t* _plen) {
...
void TXCAutoBuffer::__FitSize(size_t _len) {
    if (_len > capacity_) {
