...
TXCLogCrypt* TXCLogBuffer::s_log_crypt =  new TXCLogCrypt();

bool TXCLogBuffer::GetPeriodLogs(const char* _log_path, int _begin_hour, int _end_hour, unsigned long& _begin_pos, unsigned long& _end_pos, std::string& _err_msg) {
    return s_log_crypt->GetPeriodLogs(_log_path, _begin_hour, _end_hour, _begin_pos, _end_pos, _err_msg);
}


bool TXCLogBuffer::Write(const void* _data, size_t _inputlen, void* _output, size_t& _len) {
...
TXCLogBuffer::TXCLogBuffer(void* _pbuffer, size_t _len, bool _isCompress)
...
TXCLogBuffer::~TXCLogBuffer() {
    if (Z_NULL != cstream_.state) {
        deflateEnd(&cstream_);
    }
}

TXCPtrBuffer& TXCLogBuffer::GetData() {
    return buff_;
}


void TXCLogBuffer::Flush(TXCAutoBuffer& _buff) {
...
bool TXCLogBuffer::Write(const void* _data, size_t _length) {
...
void TXCLogBuffer::__Clear() {
    memset(buff_.Ptr(), 0, buff_.MaxLength());
    buff_.Length(0, 0);
}


void TXCLogBuffer::__Fix() {
